...
public class TelaLogin extends JFrame {
    ...
    public TelaLogin() {
        ...
        // --- Espaço/Placeholder para futura logo na tela de login ---
        gbc.gridx = 0;
        gbc.gridy = 0;
        gbc.gridwidth = 2;
        gbc.anchor = GridBagConstraints.CENTER;
        
        JPanel pnlLogoPlaceholder = new JPanel() {
            @Override
            protected void paintComponent(Graphics g) {
                super.paintComponent(g);
                Graphics2D g2d = (Graphics2D) g;
                ...
                float[] dashedPattern = {5f, 5f};
                g2d.setStroke(new BasicStroke(2f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1.0f, dashedPattern, 0.0f));
                g2d.drawRoundRect(0, 0, getWidth() - 1, getHeight() - 1, 10, 10);
            }
        };
        pnlLogoPlaceholder.setPreferredSize(new Dimension(160, 75));
        pnlLogoPlaceholder.setOpaque(false);
        pnlLogoPlaceholder.setLayout(new BorderLayout());
        
        JLabel lblLogoText = new JLabel("SUA LOGO AQUI", SwingConstants.CENTER);
        lblLogoText.setFont(new Font("Segoe UI", Font.BOLD, 11));
        lblLogoText.setForeground(new Color(189, 195, 199));
        pnlLogoPlaceholder.add(lblLogoText, BorderLayout.CENTER);
        painelPrincipal.add(pnlLogoPlaceholder, gbc);
    ...
    private void realizarLogin() {
        ...
        if (autenticacao.autenticar(login, senha)) {
            new TelaPDV(login).setVisible(true);
            dispose();
        } else {
